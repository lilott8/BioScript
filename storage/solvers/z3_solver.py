...
class Z3Solver(BaseSolver):
    ...
    def __init__(self, config):
        ...
        self.table = EpaManager.build_interaction_table(self.config.abstract_interaction)
    ...
    def validate(self, t1, t2) -> bool:
        ...
        if t1 in self.table:
            if t2 in self.table[t1]:
                return False
        # Check the inverse.
        if t2 in self.table:
            if t1 in self.table[t2]:
                return False
        return True

    def solve_constraints(self, safe_funct=lambda x, y: False):
        """
        ...
        """
        json_data = {}
        with open(self.config.input) as file:
        ...
        answer = solver.check() == z3.sat

        if not answer:
            return "ChemStor couldn't find a safe, valid solution."
        else:
